  ...
  return frc2::cmd::RunOnce(
      [this, section, r, g, b] {
        std::unique_ptr<LedPattern> pattern =
            std::make_unique<SolidColorPattern>(
                SolidColorPattern(frc::Color(r(), g(), b()),
...
frc2::CommandPtr LedSubsystem::SetSectionToRainbow(
  ...
  return frc2::cmd::RunOnce(
      [this, section] {
        std::unique_ptr<LedPattern> pattern = std::make_unique<RainbowPattern>(
            RainbowPattern(ledStrip.GetSection(section()).GetLength()));
        ledStrip.GetSection(section()).SetPattern(std::move(pattern));
      },
      {this});
}
...
frc2::CommandPtr LedSubsystem::SetSectionToFade(std::function<int()> section,
  ...
  return frc2::cmd::RunOnce(
      [this, section, r, g, b] {
        std::unique_ptr<LedPattern> pattern = std::make_unique<FadePattern>(
...
frc2::CommandPtr LedSubsystem::SetSectionToKnightRider(
  ...
  return frc2::cmd::RunOnce(
      [this, section, r, g, b] {
        std::unique_ptr<LedPattern> pattern =
            std::make_unique<KnightRiderPattern>(
                KnightRiderPattern(frc::Color(r(), g(), b()),
                                   ledStrip.GetSection(section()).GetLength()));
        ledStrip.GetSection(section()).SetPattern(std::move(pattern));
      },
      {this});
...
frc2::CommandPtr LedSubsystem::SetSectionToChase(std::function<int()> section,
  ...
  return frc2::cmd::RunOnce(
      [this, section, r, g, b] {
        std::unique_ptr<LedPattern> pattern = std::make_unique<ChasePattern>(
            ChasePattern(frc::Color(r(), g(), b()),
                         ledStrip.GetSection(section()).GetLength()));
        ledStrip.GetSection(section()).SetPattern(std::move(pattern));
      },
      {this});
...
frc2::CommandPtr LedSubsystem::SetSectionToBlink(
  ...
  return frc2::cmd::RunOnce(
      [this, section, r, g, b, onTime, offTime] {
        std::unique_ptr<LedPattern> pattern = std::make_unique<BlinkPattern>(
            BlinkPattern(frc::Color(r(), g(), b()),
                         ledStrip.GetSection(section()).GetLength(), onTime(),
                         offTime()));
        ledStrip.GetSection(section()).SetPattern(std::move(pattern));
      },
      {this});
...
frc2::CommandPtr LedSubsystem::SetBothToBlinkYellow() {
  return frc2::cmd::RunOnce(
      [this] {
        std::unique_ptr<LedPattern> pattern1 = std::make_unique<BlinkPattern>(
            BlinkPattern(frc::Color(1.0, 1.0, 0.0),
                         ledStrip.GetSection(0).GetLength(), .1_s, .1_s));
        std::unique_ptr<LedPattern> pattern2 = std::make_unique<BlinkPattern>(
            BlinkPattern(frc::Color(1.0, 1.0, 0.0),
...
frc2::CommandPtr LedSubsystem::SetBothToBlinkRed() {
  return frc2::cmd::RunOnce(
      [this] {
        std::unique_ptr<LedPattern> pattern1 = std::make_unique<BlinkPattern>(
            BlinkPattern(frc::Color(1.0, 0.0, 0.0),
                         ledStrip.GetSection(0).GetLength(), .1_s, .1_s));
        std::unique_ptr<LedPattern> pattern2 = std::make_unique<BlinkPattern>(
            BlinkPattern(frc::Color(1.0, 0.0, 0.0),
                         ledStrip.GetSection(1).GetLength(), .1_s, .1_s));
...
frc2::CommandPtr LedSubsystem::SetBothToOff() {
  return frc2::cmd::Sequence(
      SetSectionToColor([] { return 0; }, [] { return 0.0; }, [] { return 0; },
                        [] { return 0.0; }),
      SetSectionToColor([] { return 1; }, [] { return 0.0; }, [] { return 0; },
                        [] { return 0.0; }));
}
